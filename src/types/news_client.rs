...
pub trait NewsClient {
    ...
    async fn fetch_general(&self) -> Result<Vec<Arc<dyn NewsArticle>>, String>;
}
...
impl NewsClient for GNewsClient {
    ...
    async fn fetch_general(&self) -> Result<Vec<Arc<dyn NewsArticle>>, String> {
        let response = self
            .fetch_top_headlines(Some("general".into()), None, Some("us".into()), None)
            .await;
